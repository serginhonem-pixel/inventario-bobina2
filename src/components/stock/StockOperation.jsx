...
const StockOperation = ({ items, schema, tenantId, currentStockPoint }) => {
  ...
  const currentQty = useMemo(() => {
    ...
    return Number(selectedItem.data.quantidade || selectedItem.data.estoque || 0);
  }, [selectedItem]);
      ...
      <div className="flex flex-col lg:flex-row gap-4 items-start justify-between bg-zinc-900 p-6 rounded-3xl border border-zinc-800 shadow-xl">
        ...
        <div className="flex flex-1 lg:max-w-lg gap-3 w-full">
          <form onSubmit={handleSearch} className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="Bipe ou digite ID, nome ou código..." 
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center justify-between bg-zinc-950 rounded-2xl p-4 border border-zinc-800">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-zinc-300">Quantidade no sistema</span>
                </div>
                <span className="text-lg font-black text-white">{currentQty}</span>
              </div>
              <div className="flex items-center justify-between bg-zinc-950 rounded-2xl p-4 border border-zinc-800">
                <div className="flex items-center gap-3">
          ...
          {selectedItem ? (
            ...
            <div className="bg-zinc-900 border border-zinc-800 border-dashed rounded-3xl p-16 flex flex-col items-center justify-center text-center space-y-5">
              ...
              <div className="max-w-xs space-y-2">
                ...
                <p className="text-zinc-500 text-sm">Bipe uma etiqueta ou busque um item para iniciar a contagem no ponto.</p>
              </div>
            </div>
          )}
        </div>
